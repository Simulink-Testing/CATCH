...
function sgtest(skip_first)
    ...
    %         total_time = toc();
            ...
            if ~ sim_res
                ...
                % 所以其实slforge有做部分关于e.identifier的辨别
                % 1.打开一个自己的文件
                efile = fopen('demo.m','a+');
                % 2.输出 e.identifier 到文件中
                now_time = now;
                now_str = datestr(now_time, 'yyyy-mm-dd HH:MM:SS');
                fprintf(efile,e.identifier);
                fprintf(efile,'  ');
                fprintf(efile,now_str);
                fprintf(efile,'  ');         
                fprintf(efile,strcat(REPORTS_BASE,'/',model_name));
                fprintf(efile,'\n');
                % 3.最后的最后关闭文件
                fclose(efile);
